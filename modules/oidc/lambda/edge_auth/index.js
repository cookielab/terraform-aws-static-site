...
exports.handler = (event, context, callback) => {
  try {
    console.log('Edge Lambda - Starting execution');
    console.log('Edge Lambda - Received event:', JSON.stringify(event, null, 2));

    // Kontrola formátu události
    ...
    console.log('Edge Lambda - Headers:', JSON.stringify(headers, null, 2));

    // Extrakce session cookie
    let session = null;
    if (headers.cookie && Array.isArray(headers.cookie)) {
      ...
      for (const cookie of headers.cookie) {
        ...
        // Rozdělit více cookies oddělených středníkem a mezerou
    ...
    if (session) {
      ...
      if (!session.includes('.')) {
        ...
        if (providerKey && config[providerKey]) {
          ...
          // Pokud providerKey není přítomen, zkusíme všechny providery
    ...
    // Pokud není session nebo je neplatná, přesměrujeme na OIDC
